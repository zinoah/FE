...
document.getElementById("check2").addEventListener("click", function () {
  ...
  const regEx3 = /^[abcd]/;
  div1.innerHTML += "/^[abcd]/ , kitty : " + regEx3.test("group") + "<hr>";
  ...
  // ^[a-zA-ZO-9]$ : 한글과 특수문자를 제외한 모든
});

document.getElementById("inputName").addEventListener("keyup", function () {
  const rs = document.getElementById("result1");
  ...
  // 요소.classList.togle("클래스명") : 클래스가 있으면 제거 없으면 추가

  if (regEx.test(inputName.value)) {
    rs.innerText = "유효한 이름 형식입니다.";
    rs.classList.add("confirm");
    rs.classList.remove("error");
  } else {
    rs.innerText = "잘못된 형식입니다.";
    rs.classList.add("error");
    rs.classList.remove("confirm");
  }
});
